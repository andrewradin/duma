...
allWSIDs = {str(rec.agent.drugbank_id): str(rec.id) for rec in WsAnnotation.objects.filter(ws_id=ws_id)}
kt_ids=[]
for d in drugBankIds:
    if d in list(allWSIDs.keys()):
        kt_ids.append(allWSIDs[d])
    else:
        warning(d, " not found in workspace")

printOut("\t".join(kt_ids))
