...
def convertPCIDtoWSAWithSELabels(labelsFile, wsAnnotToPCID, includeNoLabels = False):
...
#    # for speed of learning, we're going to filter the features, as a lot of them were single instances
    ...
    for colInd in range(len(features[0])):
        ...
        firstPortion = getFirstPortionInList(column)
        if firstPortion < minFtPortion or (1 - firstPortion) < minFtPortion:
...
def checkSEPrevalance(sideEffects, iter, sideEffectPresence):
    ...
    firstPortion = getFirstPortionInList(seCol)
    if firstPortion < minSePortion or (1 - firstPortion) < minSePortion:
        verboseOut(se, " was not balanced enough to use. Skipping")
        return False
    return se

def buildSeSpecificAttributes(mainAttributes, se):
...
def buildSeData(features, wsas, trainingPortion, drugsWithLabels, colInd, forTesting=False):
    ...
    # first pull out which drugs in features have labels
    haveLabel = []
    for i in range(len(wsas)):
        wsa = wsas[i]
        if wsa in list(drugsWithLabels.keys()):
            haveLabel.append(i)
    ...
    for j in range(len(data)):
        # now add the SE label
        if wsas[j] in list(drugsWithLabels.keys()):
            data[j] = data[j] + "," + drugsWithLabels[wsas[j]][colInd]
        else: # otherwise this must be a drug without a label, so we don't know if it has the SE. The weka filler for missing data is ?
            data[j] = data[j] + "," + '?'
        ...
        # if we are buildiing for testing, only the data with a label, but not in training goes in the predictionData set
...
def makePreds(sideEffects, drugsWithLabels, mainAttributes, features, drugIDs, trainingPortion, forTesting, mlMethod, cleanUp):
    allPreds={}
    for i in range(len(sideEffects)):
        se = checkSEPrevalance(sideEffects, i, list(drugsWithLabels.values()))
        if not se:
            continue
        ...
        seSpecificAttributes = buildSeSpecificAttributes(mainAttributes, se)
        ...
        trainingData, predictionData, predictionDrugIds = buildSeData(features, drugIDs, trainingPortion, drugsWithLabels, i, forTesting)
...
opts.add_option("--ml_method", help="Machine learning method. Options: RF (random forest), RF_tune (tune the number of features to use), decorate (running RF), naiveBayes [DEFAULT]", default='naiveBayes')
...
opts.add_option("--detailScores", action="store_true", dest="detailScores", help="Report score for every SE contributing to a drugs' total score")
...
drugsWithLabels, sideEffects = convertPCIDtoWSAWithSELabels(options.l, getWSAtoPCID(wsId))

# retreive the features to be used (first pass, moleular bits)
features, drugIDs, bits = getFeatures(molBitsFile, list(drugsWithLabels.keys()), minFeaturePortion)
...
# keyed on the se
allProcessedWekaPredictions = makePreds(sideEffects, drugsWithLabels, mainAttributes, features, drugIDs, trainingPortion, forTesting, mlMethod, cleanUp)
...
reportScores(predictScores, realScores, sideEffectOrder, options.detailScores, filePrefix)
