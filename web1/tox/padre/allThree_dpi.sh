dpi_file=~/2xar/ws/dpi/dpi.drugbank.default.tsv
allRelevantDrugsFile="allRelevantDrugBanks_forTestingDPI.txt"
test_set_size=1
se_file=${2}
test_name=${1}
...
# first for the direct
        ...
        --d_a_file ${newDpi} --d_a_type protein --adr ${se_file} -o ${dir_prefix} \
...
# we use the full DPI file now to ensure the heldout drugs have DPI
        ...
        python napa_predict.py --d_a_file ${dpi_file} --d_a_type protein\
...
Rscript ${test_name}_path 50 paths
Rscript ${test_name}_q 50 q
Rscript ${test_name}_or 50 oddsRatio
