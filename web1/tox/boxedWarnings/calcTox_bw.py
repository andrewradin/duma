...
# This is to predict whether or not a drug from chembl has a black box warning
# the proper backgroudn for this is approved drugs (i.e. max phase 4), as only those can have a boxed warning
...
if __name__=='__main__':
    ...
    arguments = argparse.ArgumentParser(description="Predict boxed warning labels")
    
    arguments.add_argument("--attributes", help="All attributes file")
    
    arguments.add_argument("--trainingPortion", help="Portion of drugs to use for training set [0-1, DEAFULT: 0.7]", type=float, default = 0.7)
    ...
    tox_label_at_name = 'blackbox_warning'
    chemblFileName = 'create.chembl.full.tsv'
    trainingArff = os.path.splitext(args.attributes)[0] + "_training.arff"
    testingArff = os.path.splitext(args.attributes)[0] + "_testing.arff"
    outFilePrefix = 'black_box_predictions'
    ...
    allData = getAllFts(args.attributes)
    ...
    unbalancedLabels = {drug: allData[drug][tox_label_at_name] for drug in allData.keys() if tox_label_at_name in list(allData[drug].keys())}
    ...
    trainingData, testingData, trainingLabels, testingLabels = rew.split_train_test(allData, unbalancedLabels, args.trainingPortion, balanced=True)
    
    # normally I would do this first, but it takes way too long because of how many drugs there are.  So instead we will filter drugs first, and then load in the bits.
    # that will be faster, despite having to call it twice
    ...
    rew.generateArffs([item for sublist in [ftsToUse, allBitNames] for item in sublist], fullTrainingData, trainingLabels, tox_label_at_name, trainingArff, arffName='boxed_warning_training')
    rew.generateArffs([item for sublist in [ftsToUse, allBitNames] for item in sublist], fullTestingData, testingLabels, tox_label_at_name, testingArff, arffName='boxed_warning_testing')
...
#    This run is to see how we do on the testing training set
    ...
    if args.predict:
        ...
        rew.generateArffs([item for sublist in [ftsToUse, allBitNames] for item in sublist], pred_data, pred_labels, tox_label_at_name, predict_arff, label_type = ['True','False'], arffName='ptc_tox_predictions')
        ...
        rew.generateArffs([item for sublist in [ftsToUse, allBitNames] for item in sublist], combined_data, combined_labels, tox_label_at_name, combined_arff, arffName='ptc_tox_combined')
...
#        rew.generateArffs([item for sublist in [ftsToUse, allBitNames] for item in sublist], predData, predLabels, tox_label_at_name, predict_arff, arffName='ptc_tox_combined_predict')
