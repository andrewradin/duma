...
kallistoIndexDirAndPrefix="${storagePath}/forKallisto/Mus_musculus.GRCm38.rel79.cdna_kmer"
...
singleEndReadFragmentSd=20 # since we don't have that information, I'm going to just give a best guess. This is 10% of a normal range for an Illumina library. Not ideal to guess, but we dont have a choice here
...
function runKallisto {
    ...
    then
        ...
        ${SCRIPTS_PATH}/kallisto/build/src/kallisto quant -i ${kallistoIndexDir}${kmer} -o ${oDir}/${prefix} --single -l ${singleEndReadFragmentSize} -s ${singleEndReadFragmentSd} "${FASTQ1}"
        #rm "${FASTQ1}"
    elif [ "${readType}" -eq 2 ]
    then
        prefix=$(basename "${FASTQ1}" | sed 's/1_val_1\.fq\.gz//g')
        FASTQ2Name=$(basename "${FASTQ1}" | sed 's/1_val_1\.fq\.gz/2_val_2\.fq\.gz/g')
...
then
    ...
    parallel "-j${NumberOfThreads}" 'runKallisto {}' ::: $(find "${FASTQ_DIR}" -name '*1_val_1.fq.gz') ::: "${kmerBreakPoint}" ::: "${kallistoIndexDirAndPrefix}" ::: "${SCRIPTS_PATH}" ::: "${singleEndReadFragmentSize}" ::: "${singleEndReadFragmentSd}" ::: 2 ::: "${oDir}"
else
    echo "Running SE with:"
    echo $(find "${FASTQ_DIR}" -name '*_trimmed.fq.gz')
    parallel "-j${NumberOfThreads}" 'runKallisto {}' ::: $(find "${FASTQ_DIR}" -name '*_trimmed.fq.gz') ::: "${kmerBreakPoint}" ::: "${kallistoIndexDirAndPrefix}" ::: "${SCRIPTS_PATH}" ::: "${singleEndReadFragmentSize}" ::: "${singleEndReadFragmentSd}" ::: 1 ::: "${oDir}"
fi
...
rm remainingHeader.txt header.txt temp.tsv header2.txt
