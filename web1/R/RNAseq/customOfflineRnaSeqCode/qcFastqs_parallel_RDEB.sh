...
function qcFastqs {
    ...
    minLength="${5}"
    FASTQ2Name=$(basename "${FASTQ1}" | sed 's/_R1\.fastq\.gz/_R2\.fastq\.gz/g')
...
#myarray=(`find ./ -maxdepth 1 -name "*_R1.fastq.gz"`)
#if [ ${#myarray[@]} -eq 0 ]
#then
    parallel "-j${NumberOfThreads}" 'qcFastqs {}' ::: $(find "${FASTQ_DIR}" -name '*_R1.fastq.gz') ::: "${SCRIPTS_PATH}" ::: "${phredQualityCutoff}" ::: "${adaptorMatchingStringency}" ::: "${minimumReadLength}"
#else
