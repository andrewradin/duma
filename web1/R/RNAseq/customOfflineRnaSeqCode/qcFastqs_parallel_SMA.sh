...
###
# This was hardcoded to expect and work with paired end - only applicable w/Cohort1. Cohort 2 is SE
# Also changed the file endings to support what we got
###
...
function qcFastqs {
    ...
    minLength="${5}"
    FASTQ2Name=$(basename "${FASTQ1}" | sed 's/_R1_001\.fastq\.gz/_R2_001\.fastq\.gz/g')
...
parallel "-j${NumberOfThreads}" 'qcFastqs {}' ::: $(find "${FASTQ_DIR}" -name '*_R1_001.fastq.gz') ::: "${SCRIPTS_PATH}" ::: "${phredQualityCutoff}" ::: "${adaptorMatchingStringency}" ::: "${minimumReadLength}"
