...
#    FASTQ2="${FQ_DIR}/${FASTQ2Name}"
    ...
    programCall="${SCRIPTS_PATH}/trim_galore_zip/trim_galore"
...
#myarray=(`find ./ -maxdepth 1 -name "*_1.fastq.gz"`)
#if [ ${#myarray[@]} -eq 0 ]
#then
#    parallel "-j${NumberOfThreads}" 'qcFastqs {}' ::: $(find "${FASTQ_DIR}" -name '*_1.fastq.gz') ::: "${SCRIPTS_PATH}" ::: "${phredQualityCutoff}" ::: "${adaptorMatchingStringency}" ::: "${minimumReadLength}"
#else
