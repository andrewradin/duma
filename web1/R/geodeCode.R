# this was downloaded from https://github.com/nasb-course/GeoDE/archive/master.zip on 9 August 2015
# For the time being I just modified things as I wanted
# At some point it would be good to actually have this be a github repository, or talk to Avi about how best to deal with this
...
#
############################################
# load up the necessary supporting libraries
require(MASS)
require(Matrix)
...
chdirAnalysis <- function(datain, sampleclass, gammas = list(1.0), nnull = 10, CalculateSig = FALSE, includePlots = TRUE){
  ...
  if(length(datain[sampleclass==1])<2) stop ("too few control samples")
  ...
  # produce plots
   # I was getting some datasets failing, so I included some of these conditionals here to deal with that
  if(includePlots){
      print("plotting the characteristic direction...")
      if( chdirresults[[6]][[1]] == 0){
#      if( length(chdirresults[[7]][[1]] ) == 0){
          print("GeoDE found no significant probes/rows. Quitting now.")
          quit('no', as.numeric(exitCodes['noGenesFromGeoDE']))
      }else{
  ...
  if(CalculateSig){
        outSig <- mapply( function(x,ns) {
                        ...
                        , chdirresults[[1]], chdirresults[[6]], SIMPLIFY = FALSE
                        )
    list(chdirprops = chdirresults, results = outSig)
#
#    list(chdirprops = chdirresults, results = outAll, pvals = chdirresults[[7]])
  }else{
    list(chdirprops = chdirresults, results = outAll, pvals = NA)
  }
}
...
chdirplots <- function(chdirresults, sampleclass, gammas = list(1.0), CalculateSig = FALSE){
  ...
  mapply(function(a,g){
    ...
    plot(chdirresults[[2]][,1],chdirresults[[2]][,2]
        , pch = 20
        , xlab = "PC1"
        , ylab = "PC2"
        , main = paste("2D projection of CD: gamma = ", as.character(g))
        , col = as.numeric(sampleclass)
    )
    legend('topright', legend = c("Control", "Case"), col = c(1,2), pch = 20)
...
# Input:
# 1) expression data with genes down the rows and samples across the columns
# 2) a factors describing which samples are control (1) and case (2)
...
chdirSig <- function(data, sampleclass, gammas = list(1.0), nnull = 10, CalculateSig = FALSE){
  ...
  #
  # Now calculate the characteristic direction in pca space
  #
  ...
  # a matrix where rows are probes and columns are the PC values
  ...
  # rows are samples and columns are the PC values
  R <- pca1$x[,1:keepPC]
  # PC values by PC values
  ...
  if (CalculateSig){
    ...
    nu <- n1+n2-2 # number of samples - 2
    # nnull samples are drawn from a multivariate norm dist, and all of the variables have a mean of 0
    # That is then multiplied by a random chisq distribution with nnull observations and nu degrees of freedom (determined by the number of cases/controls)
...
#    bmeanvec <- colMeans(R[sampleclass==2,])- colMeans(R[sampleclass==1,])
    ...
    ratio <- mapply(function(ba,bn,count) 
                    ...
                    , b,nullchdirs, c(1:length(nullchdirs))
                    , SIMPLIFY = FALSE
             )
    nsiggenes <-lapply(ratio,function(x) which.max(x))

    ###########################################
    # Use the null distributions to calcualate p-values
    ###########################################
    Fn <- ecdf(nullchdirs[[1]])
    pvals <- lapply(b, function(ba) 
                       {ps <- Fn(sort(ba^2, decreasing = TRUE))
                        ps <- unlist(lapply(ps, function(p)
                                                {if(p == 0){p <- 1/(length(nullchdirs[[1]])+1)}
                                                 pval <- p
                                                }
                              ))
                       }
    ...
    list(  chdir = b
         ...
         , null_rank_dist = nullchdirs
         , ratio_to_null = ratio
         , number_sig_genes = nsiggenes
         , pvals = pvals
        )
  }else{
