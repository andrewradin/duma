# settings
fdrValToPlot <- 0.05
nodeSize <- 10 # 10 is the default, but because we are working with genes and not probes, I decreased it
...
allRes <- GenTable(myGOData, classicFisher = resultFisher, classicKS = resultKS, orderBy = "classicFisher", ranksOf = "classicKS", topNodes = totalNumberOfTerms)
...
allRes$foldChange <- round(allRes$Significant / allRes$Expected, 3)
